-- ============================================================================
-- FACILITY → SITE SCHEMA MIGRATION - IDEMPOTENT VERSION
-- This version safely handles partial migrations and existing constraints
-- ============================================================================

-- PHASE 1: DROP VIEWS AND POLICIES (safe to repeat)
...
-- PHASE 3: DROP ALL CONSTRAINTS WITH facility_id (handles partial migrations)
DO $$ DECLARE
    r RECORD;
BEGIN
    FOR r IN (
...
-- PHASE 6: DROP EXISTING site_id CONSTRAINTS (to re-create them cleanly)
DO $$ DECLARE
    r RECORD;
BEGIN
    FOR r IN (
        ...
        WHERE constraint_type = 'FOREIGN KEY'
        AND table_schema = 'public'
        AND constraint_name LIKE '%site_id_fkey%'
    ) LOOP
        EXECUTE format('ALTER TABLE IF EXISTS public.%I DROP CONSTRAINT IF EXISTS %I', r.table_name, r.constraint_name);
    END LOOP;
END $$;

-- PHASE 7: ADD FOREIGN KEY CONSTRAINTS USING site_id (idempotent)
